...
while ~converged
    ...
    for ff=1:F_num
        ...
        for kk=1:K
            ...
            gamma_with_tau_kk=gamma_with_tau( index_active ,kk);
            length_kk=length(gamma_with_tau_kk);           
            deltal=[gamma_with_tau_kk(2:length_kk );gamma_with_tau_kk(1)];
            deltar=[gamma_with_tau_kk(end);gamma_with_tau_kk(1:length_kk-1)];
            Z_kk=Z(1:length_kk,:,kk);
            gamma_with_tau_temp(1:length_kk,kk)=G(ff,kk)*(Z_kk(:,1).*deltar + Z_kk(:,2).*gamma_with_tau_kk +  Z_kk(:,3).*deltal );
        end
    ...
    if iter>10
        ...
        for ff=1:F_num
            for kk=1:K
                ...
                R_kk= R_all(kk);
                index_active=1 :record_num(kk);                              
                ln_delta_with_ones_kg=ln_delta_with_ones( index_active ,kk);             
                ...
                gamma_with_tau_k3=gamma_with_tau( index_active ,kk);     
    ...
    for kk=1:K
        ...
        R_kk= R_all(kk);
        index_active=1 :record_num(kk);
        ln_delta_with_ones_kk(index_record_all(index_active,kk),:)=ln_delta_with_ones( index_active ,kk);
        ...
        gamma_with_tau_kk1(index_record_all(index_active,kk),:)=gamma_with_tau( index_active ,kk);
        ...
        t1=t1( index_record_all(index_active,kk ) );t2=t2( index_record_all(index_active,kk ) );t3=t3( index_record_all(index_active,kk ) );
        t11=sum(reshape(t1,R_kk,L_kk),2);t22=sum(reshape(t2,R_kk,L_kk),2);t33=sum(reshape(t3,R_kk,L_kk),2);
        et=[t11,t22,t33]/2;
        max_et=max(et')';
        et=et-max_et*ones(1,3);
        temp_p= exp(et);
        Z_kk_old=Z_core(1:length(temp_p),:,kk);
        Z_kk= ((  1./sum(temp_p,2) )*ones(1,3)).*  temp_p;
        Z_core(1:length(temp_p),:,kk)=(1-rho)*Z_kk + rho*Z_kk_old;  % damping
        Z(1:L_kk*R_kk,:,kk)=kron( Z_core(1:R_kk,:,kk) , ones(L_kk,1) );
    end
    ...
    for kk=1:K
        ...
        if iter>=20
            ...
            gamma_core(ind_sort(1:round(end*0.8)),kk)=1e4;
            if iter>=maxiter-10
        ...
        delta_11_kk=delta_11( index_active);
    ...
    erro=norm(sum(Y_res,2) - sum(Y_res_old))/norm(sum(Y_res,2));
    if iter >= maxiter || erro <10e-6
        converged = true;
    end
    iter = iter + 1;
