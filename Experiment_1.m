...
%%   Perform the RTEA algorithm
[x_RTEA] = RTEA(Sig_N, B,lambda);
y_RTEA1=abs(fft(abs(hilbert(x_RTEA(:,1))) -mean(abs(hilbert(x_RTEA(:,1))))  ))/(N/2);
y_RTEA2=abs(fft(abs(hilbert(x_RTEA(:,2))) -mean(abs(hilbert(x_RTEA(:,2))))  ))/(N/2);


figure(11);
subplot(3,2,1)
plot(t,x_RTEA(:,1),'black')
axis([0 1 -0.5 0.5])
ylabel('Amp.[m/s^2]')
title('a) RTEA: Impluse Outer')

subplot(3,2,2)
plot(F_area, y_RTEA1(1:2001) );
axis([0 600 0 0.015])
title('b) RTEA: Envelope Outer')

subplot(3,2,3)
plot(t,x_RTEA(:,2),'black')
axis([0 1 -0.5 0.5])
xlabel('Time [s]')
ylabel('Amp.[m/s^2]')
title('c) RTEA: Impluse Inner')

subplot(3,2,4)
plot(F_area,  y_RTEA2(1:2001))
axis([0 1000 0 0.015])
xlabel('Frequency [Hz]')
title('d) RTEA: Envelope Inner')

%% GSL
mu_GSL=GSL(Sig_N);
y_GSL=abs(fft(abs(hilbert( mu_GSL )) -mean(abs(hilbert( mu_GSL )))  ))/(N/2);
...
xlabel('Time [s]')
ylabel('Amp.[m/s^2]')
title('a) GSL: Recoved Signal')

subplot(3,2,2)
plot(F_area, y_GSL(1:2001) );
xlabel('Frequency [Hz]')
axis([0 1000 0 0.015])
title('b) GSL: Envelope Spectrum')
...
mu_BPD = IterGSS_modified(Sig_N, rho, k_sparsity, Method)';
y_BPD=abs(fft(abs(hilbert( mu_BPD )) -mean(abs(hilbert( mu_BPD )))  ))/(N/2);
...
xlabel('Time [s]')
ylabel('Amp.[m/s^2]')
title('a) BPD: Recoved Signal')

subplot(3,2,2)
plot(F_area, y_BPD(1:2001) );
axis([0 1000 0 0.015])
xlabel('Frequency [Hz]')
title('b) BPD: Envelope Spectrum')
